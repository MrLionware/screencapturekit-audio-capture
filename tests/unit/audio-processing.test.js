...
const { createCaptureContext, waitForEvent } = require('../helpers/test-context');
const { createAudioSample, createAudioPattern } = require('../helpers/factories');
const { assertAudioSample, assertNear } = require('../helpers/assertions');

test('Audio Processing', async (t) => {
  await t.test('RMS Calculation', async (t) => {
    await t.test('should calculate RMS for constant signal', (t, done) => {
    ...
    await t.test('should calculate RMS = 0 for silence', (t, done) => {
      const { capture, native } = createCaptureContext();
      capture.startCapture(100, { minVolume: 0 });
    ...
    await t.test('should handle NaN values in RMS calculation', (t, done) => {
      const { capture, native } = createCaptureContext();
      capture.startCapture(100, { minVolume: 0 });

      capture.once('audio', (sample) => {
        assert.ok(!isNaN(sample.rms), 'RMS should not be NaN');
        assert.ok(isFinite(sample.rms), 'RMS should be finite');
        capture.stopCapture();
        done();
      });
  ...
  await t.test('Peak Calculation', async (t) => {
    await t.test('should calculate peak value correctly', (t, done) => {
      const { capture, native } = createCaptureContext();
      capture.startCapture(100, { minVolume: 0 });
    ...
    await t.test('should clamp extreme peak values', (t, done) => {
      const { capture, native } = createCaptureContext();
      capture.startCapture(100, { minVolume: 0 });
  ...
  await t.test('Format Conversion', async (t) => {
    ...
    await t.test('should keep Float32 format by default', (t, done) => {
      const { capture, native } = createCaptureContext();
      capture.startCapture(100, { minVolume: 0 });
  ...
  await t.test('Volume Threshold', async (t) => {
    await t.test('should filter audio below minVolume', (t, done) => {
      const { capture, native } = createCaptureContext();
    ...
    await t.test('should emit audio above minVolume', (t, done) => {
      const { capture, native } = createCaptureContext();
  ...
  await t.test('Audio Metrics', async (t) => {
    await t.test('should calculate correct sample count', (t, done) => {
      const { capture, native } = createCaptureContext();
      capture.startCapture(100, { minVolume: 0 });
    ...
    await t.test('should calculate correct frame count', (t, done) => {
      const { capture, native } = createCaptureContext();
      capture.startCapture(100, { minVolume: 0 });
    ...
    await t.test('should calculate duration correctly', (t, done) => {
      const { capture, native } = createCaptureContext();
      capture.startCapture(100, { minVolume: 0 });
