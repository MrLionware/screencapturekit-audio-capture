...
import { AudioCaptureServer } from '../service/server';
...
export async function cleanupAll(): Promise<CleanupResult> {
  const captureInstances = AudioCapture.cleanupAll();
  const serverInstances = await AudioCaptureServer.cleanupAll();
...
export function getActiveInstanceCount(): number {
  return AudioCapture.getActiveInstanceCount() + AudioCaptureServer.getActiveInstanceCount();
}
