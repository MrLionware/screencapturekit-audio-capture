...
### Added
- **App list reuse optimization**: `getAudioApps()` and `selectApp()` now accept `appList` parameter to reuse prefetched app lists, reducing redundant native calls
- **Enhanced app selection**: `selectApp()` now supports `fallbackToFirst` and `sortByActivity` options for smarter fallback behavior
- **App object support**: `startCapture()` can now accept app objects directly (from `getApplications()` or `selectApp()`), bypassing redundant lookups
- **Permission helper enhancement**: `verifyPermissions()` now returns the `apps` array for efficient reuse in subsequent calls
- **macOS 15+ timeout protection**: Added 10-second timeout to prevent indefinite hangs caused by ScreenCaptureKit bug in +macOS 15 (Sequoia)

### Fixed
- **Stream API**: Fixed `AudioStream.stop()` to prevent "push after EOF" errors by detaching event handlers before cleanup
- **STTConverter**: Fixed object mode handling to properly accept object input from AudioStream while respecting output mode setting
- **Native cleanup**: Improved ThreadSafeFunction cleanup to prevent potential double-release issues
- **Helper process filtering**: Enhanced app filtering to exclude AutoFill, .Helper, .xpc, and other background helper processes

### Changed
- Examples updated to demonstrate efficient app list reuse patterns
- Improved error messages with macOS 15+ specific guidance
- Enhanced documentation with new API parameters and troubleshooting for macOS 15+
